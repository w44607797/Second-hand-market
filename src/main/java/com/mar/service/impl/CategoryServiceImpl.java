...
import com.mar.bean.mapper.CategoryMapper;
...
public class CategoryServiceImpl extends ServiceImpl<CategoryMapper, Category> implements CategoryService {
    ...
    public List<Category> categoryList() {
        ...
        List<Category> level1 = list.stream().filter(Category ->
            ...
            level.setCategoryChild(getChildrens(level,list));
            return level;
    ...
    //递归查找所有层级的商品
    private List<Category> getChildrens(Category root,List<Category> tree){

        List<Category> list = tree.stream().filter(Category ->{
            //比较自身id和储存的父id
            return Objects.equals(Category.getParentId(),root.getCid());
        }).map((category) -> {
            category.setCategoryChild(getChildrens(category,tree));
            return category;
        }).collect(Collectors.toList());
